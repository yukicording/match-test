...
if(isset($_POST['opponent_id'])){
    ...
    delete_iine($opponent_id, $mysqli);
}?>
...
<head>
    ...
    <div class="inbox_select">
        ...
        <div class="inbox_matti">good history</div></a>
        <a href="index.php?page=not_diplay_table">
            <div class="inbox_message">good history</div></a></div>
    <h4 class="h4_match" style="text-align:center;margin-bottom:30px;">GOOD history table</h4>
        <?php
        $iine = iine_table($mysqli);
        ?>
        <p style="text-align: center;"><?php
        if(empty($iine)) {
            echo "You haven't sent GOOD to any members yet." ;
        }?></p>      
        
       <?php foreach ($iine as $user) {?>
<?php 
$id = $user['opponent_id'];
...
<?php } ?></div>
            <div class="iine_content"　style="color:gray;">
<form action="#" method="post">
            <input type="hidden" name="opponent_id" value="<?php echo "$id" ?>">
            <input type="submit" class="submit_btn" value="取り消す">
            </form>
        </div></div>
        <!-- 下記の部分を追加 -->
<?php } ?>         <script>
