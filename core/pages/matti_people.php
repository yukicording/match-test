...
<body>
    <div class="main_inbox">
        ...
        <div class="inbox_matti_1">INBOX</div></a>
        <a href="index.php?page=matti_people">
            <div class="inbox_message_1">MATCHING</div></a></div>
    <h4 class="h4_match" style="text-align:center;">MATCHING TABLE</h4>
        <?php
        $iine = select_match($mysqli);
        ?>
        <?php
        if(empty($iine)) {
            echo "マッチングしていません" ;
...
<form action="index.php?page=new_conversation" method="post">
            <input type="hidden" name="opponent_id" value="<?php echo "$id" ?>">
                <input type="hidden" name="iine_id" value="<?php echo "$iine_id" ?>">
            <input type="submit" class="submit_btn" value="MESSEAGE">
            </form>
        </div></div>
        <!-- 下記の部分を追加 -->
<?php } ?>
